    ...
    | instr :: scode' ->
      let env', code' =
		...
		| LD x -> 
	     	...
	     	env, replace (env#loc x) s  
	  	| ST x ->
        	let s, env = (env#global x)#pop in
        	env, replace s (env#loc x)
        | STA (x, n) ->
          	let s, env = (env#global x)#allocate in
           	let push = replace (env#loc x) s in
		...
		| BEGIN (name, args, locals) ->
        	...
        	env, [Push ebp; Mov (esp, ebp); Binop ("-", M ("$" ^ env#lsize), esp)]| END -> env, [Label env#epilogue; Mov (ebp, esp); Pop ebp; Ret; Meta (Printf.sprintf "\t.set %s, %d" env#lsize (env#allocated * word_size))]
